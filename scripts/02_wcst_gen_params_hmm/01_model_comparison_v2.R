...
# Verifica presenza user_id
if (!"user_id" %in% names(stan_data)) {
  stop(
    "L'elemento 'user_id' non è presente in stan_data. Verifica il file RDS."
  )
}
if (length(stan_data$user_id) != stan_data$N) {
  stop(
    "La lunghezza di user_id (",
    length(stan_data$user_id),
    ") non corrisponde al numero di soggetti N (",
    stan_data$N,
    ")"
  )
}
...
# MODIFICA: Estrai parametri e aggiungi user_id
pars_hmm <- purrr::imap_dfr(fits_hmm, function(ft, sid) {
  extract_params_hmm(ft) %>%
    ...
    dplyr::mutate(
      subject = sid,
      user_id = stan_data$user_id[sid] # AGGIUNTO: user_id dal vettore originale
    )
})
...
pars_hmm <- pars_hmm %>%
  ...
  # MODIFICA: Riordina colonne per avere user_id subito dopo subject
  dplyr::select(subject, user_id, group, group_label, everything())
...
# Controls Patients
# 46       42
# AGGIUNTO: Verifica corrispondenza user_id
message("Prime 5 righe di pars_hmm con user_id:")
print(head(pars_hmm[, c("subject", "user_id", "group_label")], 5))
subject
user_id
group_label
# <int> <chr>                  <fct>
# 1       1 cr_gi_1994_10_14_378_f Patients
# 2       2 gr_bo_1996_07_31_547_f Patients
# 3       3 an_de_1998_11_10_289_f Patients
# 4       4 al_zu_1997_04_02_880_f Patients
# 5       5 am_gu_1999_02_11_937_f Patients
...
pars_hmm_x <- pars_hmm %>%
  dplyr::transmute(
    subject,
    user_id, # AGGIUNTO: mantieni user_id anche qui
    group,
