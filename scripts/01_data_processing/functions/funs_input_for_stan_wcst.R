...
compile_data_for_stan <- function(df) {
  ...
  for (i in seq_along(subject_names)) {
    ...
    group[i] <- unique(
      df %>% dplyr::filter(subj_name == subject_names[i]) %>% dplyr::pull(group)
    )[1]
...
process_and_prepare_stan_data <- function(DEBUGGING = FALSE) {
  ...
  # Read raw data
  raw_data_patients_df <- rio::import(here::here(
    "data",
    "processed",
    "interim",
    "raw_data_patients.csv"
  ))
  raw_data_controls_df <- rio::import(here::here(
    "data",
    "processed",
    "interim",
  ...
  # Import final data
  final_data <- rio::import(here::here(
    "data",
    "processed",
    "interim",
    "raw_data_project.csv"
  ))

  # Group subjects by their group and select unique subjects for each group
  code_psytoolkit_selected_an <- unique(
    final_data[final_data$group == "an", ]$subj_name
  )
  code_psytoolkit_selected_hc <- unique(
